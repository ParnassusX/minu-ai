...
export async function POST() {
  try {
    ...
    // Since we can't execute raw DDL through Supabase client directly,
    // we'll provide the SQL for manual execution and verify the results

    // First, let's check if the prompts table already exists
    ...
    if (!checkError) {
      return NextResponse.json({
        success: true,
        message: 'Migration already applied - prompts table exists',
    ...
    if (!checkError.message.includes('does not exist')) {
      return NextResponse.json({
        error: 'Unexpected error checking prompts table',
        details: checkError.message
      }, { status: 500 })
    }

    console.log('Prompts table does not exist, migration needed')
